...
async def main(executor):
    async def per_config(x):
        ...
        for mode, clksigs in [("DP16K_MODE", ["CLKA", "CLKB"]),
            ...
            for sig in clksigs:
                fuzz_enum_setting("{}.{}.{}MUX".format(ebr, mode, sig), ["0", sig, "INV"],
                    lambda x,sig=sig,mode=mode: get_substs(mode=mode, kv=(sig, x), mux=True), False,
        ...
        for mode, cwesigs in [("DP16K_MODE", ["WEA", "WEB", "CEA", "CEB", "RSTA", "RSTB", "ADA0", "ADA1", "ADA2", "ADA3", "ADB0", "ADB1"]),
            ...
            for sig in cwesigs:
                fuzz_enum_setting("{}.{}.{}MUX".format(ebr, mode, sig), [sig, "INV"],
                    lambda x,sig=sig,mode=mode: get_substs(mode=mode, kv=(sig, x), mux=True), False)
        ...
        for mode, outregs in [("DP16K_MODE", ["OUTREG_A", "OUTREG_B"]),
            ...
            for outreg in outregs:
                fuzz_enum_setting("{}.{}.{}".format(ebr, mode, outreg), ["USED", "BYPASSED"],
                    lambda x,outreg=outreg,mode=mode: get_substs(mode=mode, kv=(outreg, x)), False,
        ...
        for mode, ports in [("DP16K_MODE", ["_A", "_B"]),
                                ...
                                #("FIFO16K_MODE", ["_A", "_B"])]:
            for port in ports:
                fuzz_enum_setting("{}.{}.RESETMODE{}".format(ebr, mode, port), ["ASYNC", "SYNC"],
                    lambda x,port=port: get_substs(mode=mode, kv=("RESETMODE{}".format(port), x)), False)
                fuzz_enum_setting("{}.{}.ASYNC_RST_RELEASE{}".format(ebr, mode, port), ["ASYNC", "SYNC"],
                    lambda x,port=port: get_substs(mode=mode, kv=("ASYNC_RST_RELEASE{}".format(port), x)), False)
        ...
        for mode, csds in [("DP16K_MODE", ["CSDECODE_A", "CSDECODE_B"]),
                                ...
                                ("SP16K_MODE", ["CSDECODE"]),]:
            for csd in csds:
                fuzz_word_setting("{}.{}.{}".format(ebr, mode, csd), 3,
                    lambda x,csd=csd,mode=mode: get_substs(mode=mode, kv=(csd, "".join(reversed(["1" if b else "0" for b in x])))),
                    desc="port is enabled when CS inputs match this value".format(csd))

        fuzz_enum_setting("{}.GSR".format(ebr), ["ENABLED", "DISABLED"],
            lambda x: get_substs(mode="DP16K_MODE", kv=("GSR", x)), False,
            desc="if `ENABLED`, then read ports are reset by user GSR")
        fuzz_enum_setting("{}.INIT_DATA".format(ebr), ["DYNAMIC", "STATIC", "NO_INIT"],
            lambda x: get_substs(mode="DP16K_MODE", kv=("INIT_DATA", x)), False,
            desc="selects initialisation mode")
        fuzz_word_setting("{}.WID".format(ebr), 11,
        ...
        fuzz_word_setting("{}.FIFO16K_MODE.EMPTYBITS".format(ebr), 5,
            lambda x: get_substs(mode="EBR_CORE", kv=("EMPTY", "0b" + "".join(reversed(["1" if b else "0" for b in x])))),
            desc="FIFO 'empty' threshold")
        fuzz_word_setting("{}.FIFO16K_MODE.ALMOST_EMPTY".format(ebr), 14,
